...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await ActivitiesDBApi.findAll(req.query, { currentUser });
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'description', 'scheduled_date'];
      const opts = { fields };
      try {
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    const currentUser = req.currentUser;
    const payload = await ActivitiesDBApi.findAll(req.query, null, {
      countOnly: true,
      currentUser,
    });
